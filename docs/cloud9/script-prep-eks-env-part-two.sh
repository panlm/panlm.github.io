...
# Detect architecture
# ARCH: amd64/arm64 (Go-style, used by kubectl, eksctl, k9s, aws-iam-authenticator, kubectl-convert)
# ARCH_UNAME: x86_64/arm64 (used by kubectx, kubens, eksdemo which use x86_64 for Intel but arm64 for ARM)
MACHINE=$(uname -m)
case ${MACHINE} in
  x86_64)  ARCH="amd64"; ARCH_UNAME="x86_64" ;;
  aarch64) ARCH="arm64"; ARCH_UNAME="arm64" ;;
  *) echo "Unsupported architecture: ${MACHINE}"; exit 1 ;;
esac
echo "Detected architecture: ${MACHINE} -> ARCH=${ARCH}, ARCH_UNAME=${ARCH_UNAME}"

mv -f ~/.bash_completion ~/.bash_completion.$(date +%N) 2>/dev/null || true
# install kubectl with +/- 1 cluster version 1.28.14 / 1.29.9 / 1.30.9 / 1.31.6
# refer: https://kubernetes.io/releases/
# sudo curl --location -o /usr/local/bin/kubectl "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/${ARCH}/kubectl"
sudo curl --silent --location -o /usr/local/bin/kubectl "https://storage.googleapis.com/kubernetes-release/release/v1.30.4/bin/linux/${ARCH}/kubectl"
...
# install kubectx
curl -L "https://github.com/ahmetb/kubectx/releases/download/v0.9.5/kubectx_v0.9.5_linux_${ARCH_UNAME}.tar.gz" |tar xz -C /tmp/
sudo mv -f /tmp/kubectx /usr/local/bin/
# install kubens
curl -L "https://github.com/ahmetb/kubectx/releases/download/v0.9.5/kubens_v0.9.5_linux_${ARCH_UNAME}.tar.gz" |tar xz -C /tmp/
sudo mv -f /tmp/kubens /usr/local/bin/

# install k9s
curl -L "https://github.com/derailed/k9s/releases/latest/download/k9s_Linux_${ARCH}.tar.gz" |tar xz -C /tmp/
sudo mv -f /tmp/k9s /usr/local/bin/

# install eksdemo
curl -L "https://github.com/awslabs/eksdemo/releases/latest/download/eksdemo_$(uname -s)_${ARCH_UNAME}.tar.gz" |tar xz -C /tmp/
...
# install aws-iam-authenticator 0.6.14
wget -O /tmp/aws-iam-authenticator "https://github.com/kubernetes-sigs/aws-iam-authenticator/releases/download/v0.6.14/aws-iam-authenticator_0.6.14_linux_${ARCH}"
...
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/${ARCH}/kubectl-convert" --output-dir /tmp
curl -LO "https://dl.k8s.io/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/${ARCH}/kubectl-convert.sha256" --output-dir /tmp
